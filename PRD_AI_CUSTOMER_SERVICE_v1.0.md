...
### B.3 PRD 写了但**未完全落地**的(2026-08-20 重新核查)| # | 项 | 实际状态 | 影响 |
|---|----|---------|------|
| 1 | 南宁吃喝玩乐长篇攻略 KB | 🟡 **有 4 条短答**(`category_id=74`:美食/天气/青秀山/附近餐饮),但**长篇攻略未做** | 客户问吃喝玩乐只能短答 + 建议地图核实 |
| 2 | 掌柜统一联系方式 KB | ❌ **0 条** | 客户问"怎么联系掌柜"无具体微信/电话 |
| 3 | `kb_documents` 文档 RAG | ❌ 表不存在,grep api 也无引用 | 长篇攻略 KB 无法落地 |
| 4 | `chat.html` iframe 嵌入测试 | 🟡 嵌入代码已生成(`settings.html` Tab「🔗 网站嵌入」第 365/665 行),但**实际嵌入测试待补** | 第三方网站嵌入功能代码已可用 |
| 5 | 批量导入 1000+ 条 | 🟡 `knowledge.php` CSV 路径无明确行数上限,grep 未见 `LIMIT 1000`(PRD §9.6 写 ≤1000,**实际无强制**) | 大批量导入可能成功也可能失败,行为未严格验证 |
| 6 | §7.2 人格化验收 3 项 | ❌ 整节空白,无对应验收 | 人设功能代码存在,但验收流程缺失 |
| 7 | SaaS 套餐管理体系 | ❌ 只有 `.trae/documents/customer-service-plan-v2.md` / `v2-plan.md` 规划文档,无代码 | 多租户计费无着落 |
| 8 | AI 自动学习 KB(对话提取 FAQ) | ❌ api 全文 grep `extract_faq / auto_learn / auto_extract` 全部无 | Phase 3 仍空白 |
| 9 | Phase 3 基础数据统计(对话量、满意度) | 🟡 `admin/dashboard.html` 存在但简陋:只有 **KB 计数 + 人设状态**,**无对话量/满意度/转化率** | 运营数据缺失 |
