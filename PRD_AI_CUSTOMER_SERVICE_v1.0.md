...
- [x] 默认种子重建 + 批量向量化可用
- [x] CSV 批量导入(无代码层行数限制,2026-08-20 实测确认)
...
- 仅受 PHP `upload_max_filesize` / `post_max_size` 限制(通常 8M-128M,**无代码层行数限制**)
- UTF-8 only
...
| 7 | SaaS 套餐管理体系 | ❌ 现阶段不做 | 多租户计费无着落 |
| 8 | ~~AI 自动学习 KB(对话提取 FAQ)~~ | ❌ **已放弃**,本功能从 PRD 移除 | 无 |
| 9 | Phase 3 基础数据统计 | 🟡 `chat_logs` **已全量保存**(2026-08-20 实测 2386 条,含 visitor_hash/source_ip/role/content/created_at/tokens,公安查验可用);`dashboard.html` 只有 **KB 计数 + 人设状态**,**缺对话量/今日消息/活跃访客**统计 | 运营数据缺;**chat_logs 全量可查**但无聚合展示 |
