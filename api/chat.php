...
if ($action === 'chat') {
    ...
    if ($pipelineEnabled) {
        ...
        try {
            ...
            error_log('[chat.php] Pipeline failed, fallback to legacy: ' . $e->getMessage());
            $pipelineEnabled = false; // 继续走旧逻辑
        }
    }

    // 旧 chat.php 逻辑（Pipeline 关闭或失败时 fallback；Pipeline 成功会 chatResponse+exit 自然跳过）
